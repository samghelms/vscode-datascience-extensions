...
const main = async function(url: string, notebookPath: string) {
	const settings = await createSettings(url);
	let manager = new ServiceManager({serverSettings: settings});
		manager.ready.then(() => {
		createApp(manager, notebookPath);
	}); 
};

function createApp(manager: ServiceManager.IManager, notebookPath: string): void {
	...
	let nbWidget = docManager.openOrReveal(notebookPath) as NotebookPanel;
...
window.addEventListener('message', event => {
	...
	switch (message.command) {
			case 'connect':
				main(message.serverUrl, message.notebookPath);
				break;
	}
});
